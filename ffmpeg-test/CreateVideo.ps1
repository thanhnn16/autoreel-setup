...
# -------------------------------
# 4. Ghép các video với hiệu ứng xfade "zoomin"
# -------------------------------

# Tạo filter complex string cho xfade
$filterComplex = "[0:v]setpts=PTS-STARTPTS[0_out];"
$inputs = ""

# Thêm input sources
for ($i = 1; $i -le 8; $i++) {
    $inputs += "-i temp_videos/$i.mp4 "
}

# Xây dựng filter complex chain
for ($i = 1; $i -lt 8; $i++) {
    $prevIndex = $i - 1
    $offset = [math]::Round($times[$prevIndex] - $transitionDuration, 2)
    $filterComplex += "[$($prevIndex)_out][${i}:v]xfade=transition=zoomin:duration=$transitionDuration:offset=$offset[$($i)_out];"
}

# Xóa dấu chấm phẩy cuối cùng và escape các ký tự đặc biệt
$filterComplex = $filterComplex.TrimEnd(';')
$filterComplex = $filterComplex -replace '"', '\"'

# Ghép video với hiệu ứng xfade
$ffmpegXfade = "ffmpeg -y -threads 0 $inputs -filter_complex ""$filterComplex"" -map ""[7_out]"" -c:v libx264 -preset $preset -crf $videoQuality -r $fps -pix_fmt yuv420p -movflags +faststart final_video_no_audio.mp4"
...
if (-not (Test-Path "final_video_no_audio.mp4")) {
    Write-Error "Error: Failed to combine videos with xfade effect."
    exit 1
} else {
    Write-Host "Videos combined with xfade effect successfully."
}
